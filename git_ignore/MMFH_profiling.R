...
profvis({
  ...
  if(any(rowSums(apply(y, 1, is.na) * 1) == dim(y)[1])){
    ...
    stop(paste0("y: All values of variable ", tmp_v1, 
  ...
  if(mode(y) != "numeric"){
    ...
    warning(paste0(
      ...
      "each domain d=1, ..., D and is therefore tranformed with as.matrix(y).\n"
    ))
  }
 ...
 at least for domain ", d, ".\n"
  ...
  if(method == "ML"){ ll <- f_ML_ll(y, x_bdiag, beta = beta_new, 
                                    theta = theta_new, V_ed, order_ab, O_d, 
                                    pos_y_obs_d) }
  if(method == "REML"){ ll <- f_REML_ll(Y_obs, x_bdiag_obs, 
  ...
  lambda_bounds <- data.frame(
    "names" = levels(cut(20, breaks = c(0, seq(20, 200, by = 20)), 
                         right = TRUE)),
    "lower" = 0,
    "upper" = c(1, 1, (9:2)/10))
  ...
  if(round(lambda, digit = 4) == 0){
    warning("Fisher-Scoring step size was reduced towards 0 as elsewise ", 
