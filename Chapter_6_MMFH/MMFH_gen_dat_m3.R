...
f_prep_MMFH_m3 <- function ( D = 100, 
  ...
  V_ed <- lapply(1:D, function(a) { cov_mat_rer }) 
...
f_gen_MMFH_m3 <- function ( D = 100, 
  ...
  e <- t(sapply(1:D,  # sampling errors
                function(d) { 
                  as.vector(mvtnorm::rmvnorm(n = 1, mean = rep(0, m), 
                                             sigma = V_ed[[d]]))
                }, 
                simplify = "array"))
  y_true <- x_beta + u
...
if (FALSE) {
  out_prep_test <- f_prep_MMFH_m3( D = 100, 
                                   v_ref = c(2,3,4),
                                   cor_ref = c(.2,.3,.4), 
                                   v_rer = c(2.5,3.5,4.5), 
                                   cor_rer = c(.15,.25,.35), 
  ...
  out_test <- f_gen_MMFH_m3( D = out_prep_test$D, 
                             x = out_prep_test$x,
